...
foreach($courses as $course) {
    out($i.'/'.$numCourses.' - '.$course->fullname.' - '.$course->idnumber, '+');
    
    if(strpos(strtolower($course->shortname), 'module study area') !== false) {
    ...
    if(strpos(strtolower($course->shortname), 'course study area') !== false) {
        ...
        foreach($filters as $filter) {
            ...
            if($match) {
                ...
                if(count($months) !== 0) {
                    $record->occurrence = implode(':', $months);
                }
                break;
            }
